...
export async function submitAptitudeTest({ mockId, category, userEmail, questions }) {
  try {
    ...
    // We pass the score and some sample incorrect answers to AI for context
    ...
    // Check if a test result for this mockId already exists
    const existingTest = await db.select().from(AptitudeTest).where(eq(AptitudeTest.mockId, mockId)).limit(1);
    
    if (existingTest.length > 0) {
      console.log("Aptitude test already submitted for this mockId. Skipping insert.");
