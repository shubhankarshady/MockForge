...
export async function createInterviewWithResume(formData) {
  ...
  try {
    ...
    // Check for both mockId (camelCase) and mockid (lowercase) in different structures
    const returnedMockId = aiResponse.mockId || aiResponse.mockid || 
                          (aiResponse[0] && (aiResponse[0].mockId || aiResponse[0].mockid)) ||
                          (aiResponse.questions && aiResponse.questions[0] && (aiResponse.questions[0].mockId || aiResponse.questions[0].mockid));

    if (returnedMockId) {
      console.log("n8n handled the insertion or returned a mockId:", returnedMockId);
      
      // Check if it actually exists in our local DB to be safe
      const existing = await db.select().from(MockInterview).where(eq(MockInterview.mockId, returnedMockId)).limit(1);
      if (existing.length > 0) {
        console.log("Record already exists in local DB. Skipping insert.");
        revalidatePath("/dashboard");
        return { success: true, mockId: returnedMockId };
      }
      // If it doesn't exist locally but we have a mockId from n8n, we continue to step 4
      // and use this mockId for our local insertion.
    }
    ...
    const mockId = interviewData.mockid || interviewData.mockId || returnedMockId || uuidv4();

    // Final safety check before insertion to prevent duplicates
    const finalCheck = await db.select().from(MockInterview).where(eq(MockInterview.mockId, mockId)).limit(1);
    if (finalCheck.length > 0) {
       console.log("Record with mockId already exists. Skipping local DB insert.");
       revalidatePath("/dashboard");
